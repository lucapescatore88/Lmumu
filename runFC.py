...
fold = "/afs/cern.ch/work/p/pluca/jobs/"+dir+namebin
...
while i < len(fs) :
	...
	if((i+1)%np_per_job==0 or i==(len(fs)-1) ) :
		...
		if not restore or not os.path.isfile("/afs/cern.ch/work/p/pluca/jobs/"+namebin+"/"+jobID+"/run.sh"):
			print "---- Job " + jobID + " / " + str(ntot_jobs) + " : submit -uexe -d " + dir+namebin + " -n " + jobID + " '" + namexec + " " + sAs + " " + sfs + " -E" + obs + " -B" + str(bin) + "'"
			os.system( "~/python/submit.py -uexe -q 1nd -d " + dir+namebin + " -n " + jobID + " '" + namexec + " " + sAs + " " + sfs + " -E" + obs + " -B" + str(bin) + "'" ) 
