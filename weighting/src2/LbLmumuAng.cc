...
LbLmumuAng::LbLmumuAng( double pol, int wilson )
{
  ...
  Pb=pol;  // Do unpolarized for now, once we are sure about implementation, change this
...
double LbLmumuAng::physicsRate(float q2, float cosTheta, float cosThetaL, float cosThetaB, float phiL, float phiB)
{
  double result=0;
...
double LbLmumuAng::physicsRate(float q2, float cosTheta, float cosThetaL, float phiL)
{
  double result=0;
                ...
                for (int mp=1; mp<=2; ++mp)
                {
                  for (double lL=-0.5; lL<=0.5; ++lL)
                  {
                        tmp+=hLepton(J,m,l1,l2)*hLepton(Jp,mp,l1,l2)*
                             ...
                             TMath::Power(-1,J+Jp)*
                             hLb(J,m,lL,lj)*TComplex::Conjugate(hLb(Jp,mp,lL,ljp))*
                             WignerD::WignerDfunction(J,lj,l1-l2,cosThetaL,phiL)*
                             TComplex::Conjugate(WignerD::WignerDfunction(Jp,ljp,l1-l2,cosThetaL,phiL));
                  }
                }
