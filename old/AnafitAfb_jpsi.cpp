...
RooDataSet * getJpsisWeight(string type, string model, TFile * file = NULL)
{
	...
	else if(type == "LL") { baseCut += CutsDef::LLcut;  baseCut += (TCut)"Lb_MassConsJpsiLambda > 5500";
	...
	RooAbsPdf * jpsiKstpdf = sWjpsi->addBkgComponent("BuKst",BuKstmumuTree,10.);
	...
	//nevts->add(*(new RooRealVar(("fracKst"+type).c_str(),"",0.001,0.,1.)));}
