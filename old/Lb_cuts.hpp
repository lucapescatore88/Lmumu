...
namespace CutsDef
{
	...
	double q2min_highfirst[] = {15.0, 0.1, 2.0, 4.0, 6.0, 11.0, 15.0, 16.0, 18.0, 1.1};
	double q2max_highfirst[] = {20.0, 2.0, 4.0, 6.0, 8.0, 12.5, 16.0, 18.0, 20.0, 6.0};

	TCut highQ2 = "TMath::Power(J_psi_1S_MM/1000,2) > 9.590888711";
	TCut lowQ2 = !highQ2;
	...
	TCut TrigPassed = (TCut)( (TString)L0Passed + " && " + (TString)Hlt1Passed + " && " + (TString)Hlt2Passed );
	...
	//TCut MVAcut = "( (weight > 0.732 && TMath::Power(J_psi_1S_MM/1000,2) > 9.590888711) || (weight > 0.969 && TMath::Power(J_psi_1S_MM/1000,2) <= 9.590888711) )";
	 TCut MVAcut = "( (weight > 0.8 && TMath::Power(J_psi_1S_MM/1000,2) > 8) || (weight > 0.9 && TMath::Power(J_psi_1S_MM/1000,2) <= 8) )";
	...
	TCut MVAcut_lowSel = "( (weight > 0.8 && TMath::Power(J_psi_1S_MM/1000,2) > 11) || (weight > 0.9 && TMath::Power(J_psi_1S_MM/1000,2) <= 11) )";
	...
	TCut commonCut = TrigPassed + MVAcut;
	TCut commonCut_lowSel = TrigPassed + MVAcut_lowSel;

	TCut cutJpsi = commonCut + massCutUnblinded + jpsiCut;
	TCut cutJpsi_lowSel = commonCut_lowSel + massCutUnblinded + jpsiCut;
	TCut cutMuMu = commonCut + massCutUnblinded; //+ avoidJpsiCut

	TCut baseCutJpsi = (TCut)( (TString)massCutUnblinded + " && " + (TString)jpsiCut + " && " + (TString)jpsiTrueID );
	TCut baseCutMuMu = (TCut)( (TString)massCutUnblinded + " && " + (TString)mumuTrueID );
}
