...
int main(int argc, char **argv)
{
	...
	if(argc > 1)
	{
		for(int a = 1; a < argc; a++)
		{
			...
			if(arg.find("-f")!=string::npos) genfL = ((TString)str).Atof();
			if(arg.find("-a")!=string::npos) genafb = ((TString)str).Atof();
		}
	}
	...
	for(int i = 0; i < ntoys; i++)
	{
		...
		RooFitResult * res = safeFit(corrPdfLL,data,afb,fL);
		
		//cout << res->covQual() << "    " << res->edm() << endl;
		
		if(!bias)
		{
			if(res->covQual()!=3) nWrongErr++;
			if(res->edm()>0.01) nWrongEDM++;
			if(res->covQual()==3 && res->edm()<0.1) pass++;
			deltafL->Fill(genfL-fL->getVal());
			deltaafb->Fill(genafb-afb->getVal());
		}
		else
		{
			if(res->covQual()!=3 || res->edm()>0.1) i--;
			else
			{
				deltafL->Fill(genfL-fL->getVal());
				deltaafb->Fill(genafb-afb->getVal());
