...
int main()
{
	TFile * trainFile = TFile::Open("/afs/cern.ch/work/p/pluca/Lmumu/weighted/trainingSamples.root");
	...
	trainFile = TFile::Open("/afs/cern.ch/work/p/pluca/Lmumu/weighted/Lb2Lmumu_CL_NBweighted.root");
	...
	TLegend leg(0.3,0.65,0.7,0.9);
	leg.AddEntry(sigTrainW,"signal","F");
	leg.AddEntry(bkgTrainW,"background","F");
	bkgTrainW->SetTitle(0);
	bkgTrainW->GetXaxis()->SetTitle("MVA output");
	bkgTrainW->GetXaxis()->SetTitle("A.U.");
	bkgTrainW->Draw("");
	sigTrainW->Draw("same");
	leg.Draw();
	c->Print("SigAndBkg.pdf");
