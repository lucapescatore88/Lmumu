...
lhcbName = TPaveText(gStyle.GetPadLeftMargin() + 0.08,
                        0.87 - gStyle.GetPadTopMargin(),
                        gStyle.GetPadLeftMargin() + 0.23,
                        0.95 - gStyle.GetPadTopMargin(),
                        "BRNDC");

#lhcbName.SetTextFont(12);
...
dict = { 'B0' : 'jpsi', 'B1' : '1500_2000', 'B2' : '1100_1250', 'B3' : '1500_1600', 'B4' : '1600_1800', 'B5' : '1800_2000', 'B6' : '010_200' }
#tdict = { 'B0' : 'jpsi', 'B1' : '15.0 < q^{2} < 20.0', 'B2' : '11.0 < q^{2} < 12.5', 'B3' : '15.0 < q^{2} < 16.0', 'B4' : '16.0 < q^{2} < 18.0', 'B5' : '18.0 < q^{2} < 20.0', 'B6' : '0.1 < q^{2} < 2.0' }
tdict = { 'B0' : 'jpsi', 'B1' : '[15.0,20.0]', 'B2' : '[11.0,12.5]', 'B3' : '[15.0,16.0]', 'B4' : '[16.0,18.0]', 'B5' : '[18.0,20.0]', 'B6' : '[0.1,2.0]' }
...
if obs != "afbfL" :
	...
	q2name = tdict[folder[-2:]].replace("[","").replace("]","").replace(",","--")
	string = q2name+' &  {:5.4}_[{:5.4}]^[{:5.4}]'.format(best_coord[0], error[0]-best_coord[0], error[1]-best_coord[0])
...
htmp.GetXaxis().SetTitle("A_{FB}^{#it{l}}")
htmp.GetYaxis().SetTitle("f_{L}")
htmp.SetTitle(tdict[folder[-2:]]+" GeV^{2}/#it{c}^{4}")
htmp.Draw()

title = TPaveText( gStyle.GetPadLeftMargin() + 0.47,
                   0.84 - gStyle.GetPadTopMargin(),
                   gStyle.GetPadLeftMargin() + 0.79,
                   0.96 - gStyle.GetPadTopMargin(),
				   "BRNDC")
...
if(is1D) :
	...
	c.Print("contours_"+dict[folder[-2:]]+"_1D.png")
	c.Print("contours_"+dict[folder[-2:]]+"_1D.C")
else :
	c.Print("contours_"+dict[folder[-2:]]+"_3D.pdf")
	c.Print("contours_"+dict[folder[-2:]]+"_3D.eps")
	c.Print("contours_"+dict[folder[-2:]]+"_3D.png")
	c.Print("contours_"+dict[folder[-2:]]+"_3D.C")
